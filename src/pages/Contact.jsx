...
export default function Contact() {
  ...
  return (
    <>
      <HeroStrip title={t.contact.heroTitle} subtitle={t.contact.heroSub} />
      ...
      <section className="bg-white py-16 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.3 }}>
            <h2 className="font-heading font-semibold text-navy text-xl mb-6">{t.contact.formHeading}</h2>
            <AnimatePresence mode="wait">
              {!submitted ? (
                <motion.form key="form" initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.25 }} onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="font-body text-sm text-navy mb-1 block">{t.contact.name} <span className="text-red-500">*</span></label>
                      <input name="name" type="text" required placeholder={t.contact.namePlaceholder} value={form.name} onChange={handleChange} className={inputCls} />
                    </div>
                    <div>
                      <label className="font-body text-sm text-navy mb-1 block">{t.contact.email} <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="font-body text-sm text-navy mb-1 block">{t.contact.phone}</label>
                    <input name="phone" type="tel" placeholder={t.contact.phonePlaceholder} value={form.phone} onChange={handleChange} className={inputCls} />
                  </div>
                  <div>
                    <label className="font-body text-sm text-navy mb-1 block">{t.contact.subject} <span className="text-red-500">*</span></label>
                    <input name="subject" type="text" required placeholder={t.contact.subjectPlaceholder} value={form.subject} onChange={handleChange} className={inputCls} />
                  </div>
                  <div>
                    <label className="font-body text-sm text-navy mb-1 block">{t.contact.message} <span className="text-red-500">*</span></label>
                    <textarea name="message" rows={5} required placeholder={t.contact.messagePlaceholder} value={form.message} onChange={handleChange} className={`${inputCls} resize-none`} />
                  </div>
                  {error && <p className="font-body text-sm text-red-500">{t.contact.error}</p>}
                ...
                <motion.div key="ok" initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.3 }} className="bg-light-gray rounded-card p-8 text-center">
                  <CheckCircle size={48} className="text-green-500 mx-auto mb-4" />
                  <h3 className="font-heading font-bold text-navy text-xl mb-2">{t.contact.successHeading}</h3>
                  <p className="font-body text-sm text-muted">
                    {t.contact.thankYou}, {form.name.split(' ')[0]}. {t.contact.successSub}
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          {/* Right — Company Info */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.3, delay: 0.1 }}>
            <div className="bg-light-gray rounded-card p-6 mb-6">
              ...
              <div className="mt-6 pt-5 border-t border-mid-gray">
                <p className="font-body text-xs text-muted mb-3">{t.contact.followUs}</p>
